...
export function SyncCompanyPlan() {
  const { isAuthenticated } = useConvexAuth(); // ← Convex-level auth, not Clerk-level
  ...
  useEffect(() => {
    if (!isAuthenticated) return; // Convex has the JWT, safe to mutate
    if (!orgId || !has) return;
    ...
    syncCompanyPlan({ clerkOrgId: orgId, plan, seatLimit, jobLimit }).catch((error) => {
      syncedOrgRef.current = null;
      toast.error(getErrorMessage(error, "Could not sync company plan."));
    });
  }, [isAuthenticated, orgId, has, syncCompanyPlan]);
