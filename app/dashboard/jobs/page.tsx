...
export default function JobsPage() {
  ...
  const jobs = useQuery(api.job.searchJobListings, {
  ...
  const favorites = useQuery(api.favourites.listMyFavorites, { limit: 200 });
  const addFavorite = useMutation(api.favourites.addFavorite);
  const removeFavorite = useMutation(api.favourites.removeFavorite);
  const [statusText, setStatusText] = useState<string | null>(null);
  const [pendingFavoriteJobId, setPendingFavoriteJobId] = useState<string | null>(null);

  const favoriteJobIds = useMemo(
    () => new Set((favorites ?? []).map((item: any) => item.job?._id).filter(Boolean)),
    [favorites],
  );
    ...
    <section className="animate-fade-in space-y-6">
      ...
      <div className="space-y-3">
        ...
        {jobs?.map((job: any, index: any) => {
          ...
          const jobUrl = `/dashboard/jobs/${job._id}`;
          return (
            <div
              key={job._id}
              role="link"
